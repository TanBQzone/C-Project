...
题目：
    奖学金评选系统
        将班上同学本学期8门课程，其中包含4门核心课，其成绩全部存储到数组中，并将数组中的数据存储到文件。要求在评选奖学金时，从文件里读取全班同学的成绩，对于每位学生汇总总分及计算平均分并输出。程序最终给出评选结果，总分排名前10位的同学分别获得甲等奖学金(2位)、乙等奖学金(3位)、丙等奖学金(5位)。如果全部课程总分相同，核心课总分高的排前面，无须考虑极其特殊的情况。
